...
TEST_P(NvTiffExtEncoderTest, SingleImage)
{
    ...
    image_info_.buffer_size = height * image_info_.plane_info[0].row_stride;
    image_info_.buffer_kind = NVIMGCODEC_IMAGE_BUFFER_KIND_STRIDED_DEVICE;
    ASSERT_EQ(cudaSuccess, cudaMalloc(&image_info_.buffer, image_info_.buffer_size));
    ...
    ASSERT_EQ(cudaSuccess, cudaMemcpy(image_buffer_.data(), image_info_.buffer, image_info_.buffer_size, cudaMemcpyDeviceToHost));
    ...
    if (strcmp(std::get<0>(GetParam()), "tiff/ycck_colorspace.tiff") == 0) {
        ...
        size_t raw_size = image_info_.buffer_size;
