...
std::unique_ptr<uint8_t[]> UncompressLow(const void* srcdata, FewerArgsForCompiler* argball)
{
    ...
    if (setjmp(jpeg_jmpbuf)) {
        ...
        if (progress.scans_exceeded != 0) {
            std::stringstream ss{};
            ss << "The number of scans (" << progress.scans_exceeded
               << ") during progressive decoding of the image exceeded the currently supported value of " << progress.max_scans
               << ". If that's intentional, you can increase the limit of progressive scans by setting the environmental variable "
                  "`NVIMGCODEC_MAX_JPEG_SCANS` to the desired limit.";
